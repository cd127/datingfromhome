  ...
  $scope.showTermConditions = function() {
   ...
   $scope.closeTCModal = function() {
    ...
    $scope.openPad = function(card) {
        ...
        if(!$scope.one_point_only){
            $scope.pad.number_of_points = $scope.point_validate_interval;
        }        

        $scope.page_title = $scope.pad_title;
        var remaining = card.max_number_of_points - card.number_of_points;
        var points = [];
        console.log('point_validate_interval', $scope.point_validate_interval);
        var j = 0;
        for (var i = $scope.point_validate_interval; i <= remaining;  i += $scope.point_validate_interval) {
            points[j] = i;
            j = j+1
        }
