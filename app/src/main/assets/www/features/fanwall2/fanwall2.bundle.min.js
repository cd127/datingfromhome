angular.module('starter').controller('FanwallCommentController',function($scope){});angular.module('starter').controller('FanwallEditCommentController',function($scope,$rootScope,$stateParams,$translate,Fanwall,FanwallPost,Dialog,Picture,Loader){angular.extend($scope,{pageTitle:$translate.instant('Edit comment','fanwall'),form:{text:'',picture:'',date:null}});$scope.getCardDesign=function(){return Fanwall.getSettings().cardDesign};$scope.getSettings=function(){return Fanwall.getSettings()};$scope.picturePreview=function(){if($scope.form.picture.indexOf("/")===0){return IMAGE_URL+"images/application"+$scope.form.picture}
...
return $q.resolve()};$scope.changeLocationSettings=function(preference,reinit){$scope.preference=preference;$session.setItem($scope.preferenceKey,preference).then(function(value){if(reinit===!0){$scope.init()}}).catch(function(err){if(reinit===!0){$scope.init()}})};$scope.buildPopoverItems=function(){$scope.popoverItems=[];if(Location.isEnabled){$scope.popoverItems.push({label:$translate.instant('Locate me once','fanwall'),icon:'icon ion-android-locate',click:function(){$scope.closeActions().then(function(){$scope.changeLocationSettings('once',!0)})}});$scope.popoverItems.push({label:$translate.instant('Always ask','fanwall'),icon:'icon ion-help',click:function(){$scope.closeActions().then(function(){$scope.changeLocationSettings('ask',!0)})}});$scope.popoverItems.push({label:$translate.instant('Always locate me','fanwall'),icon:'icon ion-sb-location-on',click:function(){$scope.closeActions().then(function(){$scope.changeLocationSettings('always',!0)})}});$scope.popoverItems.push({label:$translate.instant('Never locate me','fanwall'),icon:'icon ion-sb-location-off',click:function(){$scope.closeActions().then(function(){$scope.changeLocationSettings('never',!0)})}})}else{$scope.popoverItems.push({label:$translate.instant('Check my location','fanwall'),icon:'icon ion-sb-location-off',click:function(){$scope.closeActions().then(function(){Location.requestLocation(function(){$scope.init()})})}})}};if($scope.post!==undefined){$scope.pageTitle='Edit post';$scope.form.text=$scope.post.text.replace(/(<br( ?)(\/?)>)/gm,"\n");$scope.form.pictures=$scope.post.images}
$scope.noLocation=function(){$timeout(function(){$scope.fetchingLocation=!1;$scope.form.location.latitude=0;$scope.form.location.longitude=0;$scope.shortLocation=$translate.instant('no location, check your preferences','fanwall');$scope.form.location.locationShort=$translate.instant('no location, check your preferences','fanwall')})};$scope.fetchLocation=function(){if(Location.isEnabled){$scope.fetchingLocation=!0;Location.getLocation({timeout:10000,enableHighAccuracy:!1},!0).then(function(position){$scope.form.location.latitude=position.coords.latitude;$scope.form.location.longitude=position.coords.longitude;GoogleMaps.reverseGeocode(position.coords).then(function(results){if(results.length>0){var place=results[0];try{$scope.shortLocation=_.find(place.address_components,function(item){return item.types.indexOf('locality')>=0}).long_name}catch(e){$scope.shortLocation=place.formatted_address}
...
$scope.init()}).catch(function(err){$scope.preference='always';$scope.init()})});angular.module('starter').controller('FanwallGalleryController',function($rootScope,$scope,$state,$stateParams,$timeout,$ionicScrollDelegate,Fanwall,FanwallGallery){angular.extend($scope,{isLoading:!1,collection:[],hasMore:!1,});$scope.getCardDesign=function(){return Fanwall.getSettings().cardDesign};$scope.getSettings=function(){return Fanwall.getSettings()};$scope.imagePath=function(image){if(image.length<=0){return'./features/fanwall2/assets/templates/images/placeholder.png'}
return IMAGE_URL+'images/application'+image};$scope.loadMore=function(){$scope.loadContent(!1,!0)};$scope.loadContent=function(refresh,loadMore){if(refresh===!0){$scope.isLoading=!0;$scope.collection=[];$timeout(function(){$ionicScrollDelegate.$getByHandle('mainScroll').scrollTop()})}
...
if(refresh===!0){$scope.isLoading=!1}})};$rootScope.$on('fanwall.refresh',function(){if($scope.currentTab==='gallery'){$scope.loadContent(!0)}});$scope.loadContent($scope.collection.length===0)});angular.module('starter').controller('FanwallHomeController',function($rootScope,$scope,$state,$stateParams,$translate,$timeout,$ionicSideMenuDelegate,Customer,Dialog,Location,Fanwall,FanwallUtils,GoogleMaps){angular.extend($scope,{settingsAreLoaded:!1,value_id:$stateParams.value_id,collection:[],pageTitle:'',hasMore:!1,currentTab:'post'});$scope.getCardDesign=function(){return Fanwall.getSettings().cardDesign};$scope.getSettings=function(){return Fanwall.getSettings()};$scope.locationTab=function(){return['nearby','map'].indexOf($scope.currentTab)!==-1};$scope.locationIsDisabled=function(){return!Location.isEnabled};$scope.showTab=function(tabName){$ionicSideMenuDelegate.canDragContent(tabName!=='map');var homeScope=$scope;if(tabName==='profile'&&!Customer.isLoggedIn()){return Customer.loginModal(undefined,function(){homeScope.showTab('profile')},undefined,function(){homeScope.showTab('profile')})}
...
$scope.form.date=Math.round(Date.now()/1000);$scope.instantAppend($scope.form.text);$scope.isSending=!0;return FanwallPost.sendComment($scope.post.id,null,$scope.form).then(function(payload){$scope.clearComment();$timeout(function(){$scope.post.comments=angular.copy(payload.comments);$scope.post.commentCount=$scope.post.comments.length})},function(payload){Dialog.alert('Error',payload.message,'OK',-1,'OK');$scope.post.comments.pop()}).then(function(){$scope.isSending=!1})}}}});angular.module('starter').directive('fanwallPostList',function($timeout,Lightbox){return{restrict:'E',replace:!0,templateUrl:'features/fanwall2/assets/templates/l1/tabs/directives/post-list.html',link:function(scope){scope.$watch('post',function(){scope._post=scope.post})},controller:function($scope){$scope.listDidRender=function(){$timeout(function(){Lightbox.run('.list-posts')},200)}}}});angular.module('starter').directive('fanwallPostItem',function($rootScope,$filter,$sce,$translate,$timeout,$q,Customer,Dialog,Loader,Fanwall,FanwallPost,FanwallUtils,Lightbox,Popover){return{restrict:'E',templateUrl:'features/fanwall2/assets/templates/l1/tabs/directives/post-item.html',controller:function($scope){$scope.actionsPopover=null;$scope.popoverItems=[];$scope.getCardDesign=function(){return Fanwall.getSettings().cardDesign};$scope.getSettings=function(){return Fanwall.getSettings()};$scope.userLike=function(){return $scope.getSettings().features.enableUserLike};$scope.userComment=function(){return $scope.getSettings().features.enableUserComment};$scope.showText=function(){return $filter('linky')($scope.post.text)};$scope.showLikeOrComment=function(){return($scope.post.likeCount>0||$scope.post.commentCount>0)&&($scope.canLikeOrComment())};$scope.canLikeOrComment=function(){return($scope.userLike()||$scope.userComment())};$scope.imagePath=function(path){if(path!==undefined){return IMAGE_URL+'images/application'+path}
...
Loader.show();FanwallPost.deletePost($scope.post.id,value).then(function(payload){$rootScope.$broadcast('fanwall.refresh')},function(payload){Dialog.alert('Error!',payload.message,'OK',-1,'fanwall')}).then(function(){Loader.hide()})})};$scope.commentModal=function(){if($scope.userComment()){FanwallUtils.commentModal($scope.post)}};$scope.toggleLike=function(){if(!Customer.isLoggedIn()){return Customer.loginModal()}
...
return $q.resolve()};$scope.buildPopoverItems=function(){$scope.popoverItems=[];$scope.popoverItems.push({label:$translate.instant('Unblock user','fanwall'),icon:'icon ion-android-remove-circle',click:function(){$scope.closeActions().then(function(){$scope.unblockUser()})}})};$scope.buildPopoverItems()}}});angular.module('starter').factory('Fanwall',function($pwaRequest,$stateParams){var factory={storage:[],lastValueId:null};factory.loadSettings=function(){if(factory.storage.hasOwnProperty($stateParams.value_id)){return $pwaRequest.resolve(factory.storage[$stateParams.value_id])}
