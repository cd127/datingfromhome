...
return IMAGE_URL+'images/application'+image};$scope.formIsValid=function(){var required=['number','password','text','textarea','date','datetime','clickwrap','select'];var isValid=!0;var invalidFields=[];$scope.fields.forEach(function(field){if(required.indexOf(field.type)>=0&&field.is_required){if(field.type==='clickwrap'&&field.value!==!0){invalidFields.push('&nbsp;-&nbsp;'+field.label);isValid=!1}else if(field.type==='number'){var current=parseFloat(field.value);var min=Number.parseInt(field.min);var max=Number.parseInt(field.max);var step=parseFloat(field.step);if(current<min||current>max){var text=$translate.instant('is not inside range','form2')+' '+min+'-'+max;invalidFields.push('&nbsp;-&nbsp;'+field.label+' '+text);isValid=!1}
if(!Number.isInteger(current/step)){var text=$translate.instant('must match increment','form2')+' '+step;invalidFields.push('&nbsp;-&nbsp;'+field.label+' '+text);isValid=!1}}else if(field.value===undefined||(field.value+'').trim().length===0){invalidFields.push('&nbsp;-&nbsp;'+field.label);isValid=!1}}});if(!isValid){Dialog.alert('Required fields',invalidFields.join('<br />'),'OK',-1,'form2')}
return isValid};$scope.submit=function(){Loader.show();var isValid=$scope.formIsValid();if(!isValid){Loader.hide();return}
Form2
