...
function applyEach$1(eachfn){return function(fns){var args=slice(arguments,1);var go=initialParams(function(args,callback){var that=this;return eachfn(fns,function(fn,cb){wrapAsync(fn).apply(that,args.concat(cb))},callback)});if(args.length){return go.apply(this,args)}else{return go}}}
...
var asyncTag='[object AsyncFunction]';var funcTag='[object Function]';var genTag='[object GeneratorFunction]';var proxyTag='[object Proxy]';function isFunction(value){if(!isObject(value)){return!1}
...
var objectProto$3=Object.prototype;var hasOwnProperty$2=objectProto$3.hasOwnProperty;var propertyIsEnumerable=objectProto$3.propertyIsEnumerable;var isArguments=baseIsArguments(function(){return arguments}())?baseIsArguments:function(value){return isObjectLike(value)&&hasOwnProperty$2.call(value,'callee')&&!propertyIsEnumerable.call(value,'callee')};var isArray=Array.isArray;function stubFalse(){return!1}
...
var nextElem=iterator(obj);var done=!1;var running=0;function iterateeCallback(err,value){running-=1;if(err){done=!0;callback(err)}else if(value===breakLoop||(done&&running<=0)){done=!0;return callback(null)}else{replenish()}}
function replenish(){while(running<limit&&!done){var elem=nextElem();if(elem===null){done=!0;if(running<=0){callback(null)}
return}
...
var mapLimit=doParallelLimit(_asyncMap);var mapSeries=doLimit(mapLimit,1);var applyEachSeries=applyEach$1(mapSeries);var apply=function(fn){var args=slice(arguments,1);return function(){var callArgs=slice(arguments);return fn.apply(null,args.concat(callArgs))}};function arrayEach(array,iteratee){var index=-1,length=array==null?0:array.length;while(++index<length){if(iteratee(array[index],index,array)===!1){break}}
return array}
...
var results={};var runningTasks=0;var hasError=!1;var listeners=Object.create(null);var readyTasks=[];var readyToCheck=[];var uncheckedDependencies={};baseForOwn(tasks,function(task,key){if(!isArray(task)){enqueueTask(key,[task]);readyToCheck.push(key);return}
...
uncheckedDependencies[key]=remainingDependencies;arrayEach(dependencies,function(dependencyName){if(!tasks[dependencyName]){throw new Error('async.auto task `'+key+'` has a non-existent dependency `'+dependencyName+'` in '+dependencies.join(', '))}
...
curr=next}
return this};function queue(worker,concurrency,payload){if(concurrency==null){concurrency=1}else if(concurrency===0){throw new Error('Concurrency must not be zero')}
...
q.started=!0;if(!isArray(data)){data=[data]}
...
function _next(tasks){return function(err){numRunning-=1;for(var i=0,l=tasks.length;i<l;i++){var task=tasks[i];var index=baseIndexOf(workersList,task,0);if(index>=0){workersList.splice(index,1)}
task.callback.apply(task,arguments);if(err!=null){q.error(err,task.data)}}
if(numRunning<=(q.concurrency-q.buffer)){q.unsaturated()}
if(q.idle()){q.drain()}
q.process()}}
var isProcessing=!1;var q={_tasks:new DLL(),concurrency:concurrency,payload:payload,saturated:noop,unsaturated:noop,buffer:concurrency/4,empty:noop,drain:noop,error:noop,started:!1,paused:!1,push:function(data,callback){_insert(data,!1,callback)},kill:function(){q.drain=noop;q._tasks.empty()},unshift:function(data,callback){_insert(data,!0,callback)},remove:function(testFn){q._tasks.remove(testFn)},process:function(){if(isProcessing){return}
isProcessing=!0;while(!q.paused&&numRunning<q.concurrency&&q._tasks.length){var tasks=[],data=[];var l=q._tasks.length;if(q.payload)l=Math.min(l,q.payload);for(var i=0;i<l;i++){var node=q._tasks.shift();tasks.push(node);workersList.push(node);data.push(node.data)}
...
isProcessing=!1},length:function(){return q._tasks.length},running:function(){return numRunning},workersList:function(){return workersList},idle:function(){return q._tasks.length+numRunning===0},pause:function(){q.paused=!0},resume:function(){if(q.paused===!1){return}
q.paused=!1;setImmediate$1(q.process)}};return q}
...
function _createTester(check,getResult){return function(eachfn,arr,iteratee,cb){cb=cb||noop;var testPassed=!1;var testResult;eachfn(arr,function(value,_,callback){iteratee(value,function(err,result){if(err){callback(err)}else if(check(result)&&!testResult){testPassed=!0;testResult=getResult(!0,value);callback(null,breakLoop)}else{callback()}})},function(err){if(err){cb(err)}else{cb(null,testPassed?testResult:getResult(!1))}})}}
...
var eachSeries=doLimit(eachLimit$1,1);function ensureAsync(fn){if(isAsync(fn))return fn;return initialParams(function(args,callback){var sync=!0;args.push(function(){var innerArgs=arguments;if(sync){setImmediate$1(function(){callback.apply(null,innerArgs)})}else{callback.apply(null,innerArgs)}});fn.apply(this,args);sync=!1})}
...
function timeout(asyncFn,milliseconds,info){var fn=wrapAsync(asyncFn);return initialParams(function(args,callback){var timedOut=!1;var timer;function timeoutCallback(){var name=asyncFn.name||'anonymous';var error=new Error('Callback function "'+name+'" timed out.');error.code='ETIMEDOUT';if(info){error.info=info}
...
nextTask([])};var index={applyEach:applyEach,applyEachSeries:applyEachSeries,apply:apply,asyncify:asyncify,auto:auto,autoInject:autoInject,cargo:cargo,compose:compose,concat:concat,concatLimit:concatLimit,concatSeries:concatSeries,constant:constant,detect:detect,detectLimit:detectLimit,detectSeries:detectSeries,dir:dir,doDuring:doDuring,doUntil:doUntil,doWhilst:doWhilst,during:during,each:eachLimit,eachLimit:eachLimit$1,eachOf:eachOf,eachOfLimit:eachOfLimit,eachOfSeries:eachOfSeries,eachSeries:eachSeries,ensureAsync:ensureAsync,every:every,everyLimit:everyLimit,everySeries:everySeries,filter:filter,filterLimit:filterLimit,filterSeries:filterSeries,forever:forever,groupBy:groupBy,groupByLimit:groupByLimit,groupBySeries:groupBySeries,log:log,map:map,mapLimit:mapLimit,mapSeries:mapSeries,mapValues:mapValues,mapValuesLimit:mapValuesLimit,mapValuesSeries:mapValuesSeries,memoize:memoize,nextTick:nextTick,parallel:parallelLimit,parallelLimit:parallelLimit$1,priorityQueue:priorityQueue,queue:queue$1,race:race,reduce:reduce,reduceRight:reduceRight,reflect:reflect,reflectAll:reflectAll,reject:reject,rejectLimit:rejectLimit,rejectSeries:rejectSeries,retry:retry,retryable:retryable,seq:seq,series:series,setImmediate:setImmediate$1,some:some,someLimit:someLimit,someSeries:someSeries,sortBy:sortBy,timeout:timeout,times:times,timesLimit:timeLimit,timesSeries:timesSeries,transform:transform,tryEach:tryEach,unmemoize:unmemoize,until:until,waterfall:waterfall,whilst:whilst,all:every,any:some,forEach:eachLimit,forEachSeries:eachSeries,forEachLimit:eachLimit$1,forEachOf:eachOf,forEachOfSeries:eachOfSeries,forEachOfLimit:eachOfLimit,inject:reduce,foldl:reduce,foldr:reduceRight,select:filter,selectLimit:filterLimit,selectSeries:filterSeries,wrapSync:asyncify};exports['default']=index;exports.applyEach=applyEach;exports.applyEachSeries=applyEachSeries;exports.apply=apply;exports.asyncify=asyncify;exports.auto=auto;exports.autoInject=autoInject;exports.cargo=cargo;exports.compose=compose;exports.concat=concat;exports.concatLimit=concatLimit;exports.concatSeries=concatSeries;exports.constant=constant;exports.detect=detect;exports.detectLimit=detectLimit;exports.detectSeries=detectSeries;exports.dir=dir;exports.doDuring=doDuring;exports.doUntil=doUntil;exports.doWhilst=doWhilst;exports.during=during;exports.each=eachLimit;exports.eachLimit=eachLimit$1;exports.eachOf=eachOf;exports.eachOfLimit=eachOfLimit;exports.eachOfSeries=eachOfSeries;exports.eachSeries=eachSeries;exports.ensureAsync=ensureAsync;exports.every=every;exports.everyLimit=everyLimit;exports.everySeries=everySeries;exports.filter=filter;exports.filterLimit=filterLimit;exports.filterSeries=filterSeries;exports.forever=forever;exports.groupBy=groupBy;exports.groupByLimit=groupByLimit;exports.groupBySeries=groupBySeries;exports.log=log;exports.map=map;exports.mapLimit=mapLimit;exports.mapSeries=mapSeries;exports.mapValues=mapValues;exports.mapValuesLimit=mapValuesLimit;exports.mapValuesSeries=mapValuesSeries;exports.memoize=memoize;exports.nextTick=nextTick;exports.parallel=parallelLimit;exports.parallelLimit=parallelLimit$1;exports.priorityQueue=priorityQueue;exports.queue=queue$1;exports.race=race;exports.reduce=reduce;exports.reduceRight=reduceRight;exports.reflect=reflect;exports.reflectAll=reflectAll;exports.reject=reject;exports.rejectLimit=rejectLimit;exports.rejectSeries=rejectSeries;exports.retry=retry;exports.retryable=retryable;exports.seq=seq;exports.series=series;exports.setImmediate=setImmediate$1;exports.some=some;exports.someLimit=someLimit;exports.someSeries=someSeries;exports.sortBy=sortBy;exports.timeout=timeout;exports.times=times;exports.timesLimit=timeLimit;exports.timesSeries=timesSeries;exports.transform=transform;exports.tryEach=tryEach;exports.unmemoize=unmemoize;exports.until=until;exports.waterfall=waterfall;exports.whilst=whilst;exports.all=every;exports.allLimit=everyLimit;exports.allSeries=everySeries;exports.any=some;exports.anyLimit=someLimit;exports.anySeries=someSeries;exports.find=detect;exports.findLimit=detectLimit;exports.findSeries=detectSeries;exports.forEach=eachLimit;exports.forEachSeries=eachSeries;exports.forEachLimit=eachLimit$1;exports.forEachOf=eachOf;exports.forEachOfSeries=eachOfSeries;exports.forEachOfLimit=eachOfLimit;exports.inject=reduce;exports.foldl=reduce;exports.foldr=reduceRight;exports.select=filter;exports.selectLimit=filterLimit;exports.selectSeries=filterSeries;exports.wrapSync=asyncify;Object.defineProperty(exports,'__esModule',{value:!0})})));angular.module('starter').controller('ClubcardViewController',function(Modal,$state,$stateParams,$timeout,$scope,$translate,Clubcard,$ionicNavBarDelegate,$window,Url,$ionicViewService,Customer,Loader){$scope.value_id=Clubcard.value_id=$stateParams.value_id;$scope.user={};$scope.clubcard_data={};Loader.show();function loadClubcardImage(url){return new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(new Error(`load ${url} fail`));img.src=url})}
$scope.canvasBar=function(userpoints,maxpoints,cards,progressbarColor){var canvas=document.getElementById("progressCanvas");var context=canvas.getContext('2d');var rectX=20;var rectY=50;var rectWidth=350;var rectHeight=20;var cornerRadius=20;var userpoints=userpoints;var max=maxpoints;var cards=cards;var val=Math.min(Math.max(userpoints,0),max);context.lineJoin="round";context.lineWidth=cornerRadius;context.strokeStyle='#DCDCDC';context.strokeRect(rectX+(cornerRadius/2)-2,rectY+(cornerRadius/2)+2,rectWidth-cornerRadius+4,rectHeight-cornerRadius)+4;context.strokeRect(rectX+(cornerRadius/2)-2,rectY+(cornerRadius/2)-2,rectWidth-cornerRadius+4,rectHeight-cornerRadius)+4;context.strokeStyle='#fff';context.strokeRect(rectX+(cornerRadius/2),rectY+(cornerRadius/2),rectWidth-cornerRadius,rectHeight-cornerRadius);context.strokeStyle=progressbarColor;var fillVal=Math.min(Math.max(val/max,0),1);context.strokeRect(rectX+(cornerRadius/2),rectY+(cornerRadius/2),fillVal*(rectWidth-cornerRadius),rectHeight-cornerRadius);async.each(cards,function(card,callback){loadClubcardImage(card.card_picture).then(img=>{var pos=(rectWidth*card.points/max);var position=(pos>rectWidth)?rectWidth-30:pos;context.drawImage(img,position,rectY-10);callback()})},function(err){if(err){console.log('A image failed to process')}});context.lineWidth=1;if(userpoints==0||userpoints==null){userpoints=0;context.moveTo(fillVal*(rectWidth-cornerRadius)+20,rectY+10);context.lineTo(fillVal*(rectWidth-cornerRadius)+20,10);context.stroke()}else{context.moveTo(fillVal*(rectWidth-cornerRadius)+40,rectY+10);context.lineTo(fillVal*(rectWidth-cornerRadius)+40,10);context.stroke()}
if(fillVal<0.3){context.font="20px Comic Sans MS";context.fillStyle="#000";context.fillText(userpoints,fillVal*(rectWidth-cornerRadius)+45,rectY-30);context.font="18px Comic Sans MS";context.fillText("total points",fillVal*(rectWidth-cornerRadius)+45,rectY-15)}else{context.font="20px Comic Sans MS";context.fillStyle="#000";context.fillText(userpoints,fillVal*(rectWidth-cornerRadius)-10,rectY-30);context.font="18px Comic Sans MS";context.fillText("total points",fillVal*(rectWidth-cornerRadius)-60,rectY-15)}}
document.querySelector('.auto_height').style.minHeight=window.innerHeight+'px';$scope.loadContent=function(){Clubcard.getpoints(Customer.customer.id).success(function(res){$scope.clubcard_data.behind=res.behind;$scope.clubcard_data.cards=res.cards;$scope.user.badge=res.userbadge.image||null;$scope.user.badgename=res.userbadge.name;$scope.user.points=res.userpoints||0;$scope.user.privilege=res.userbadge.privilege||null;$scope.user.note=res.userbadge.note||null;$scope.clubcard_data.terms=res.terms;$scope.clubcard_data.cover=res.cover||null;$scope.nextcard=res.nextcard;$scope.clubcard_data.becoming_message=res.becoming_message;$scope.page_title=res.page_title;var border='.Clubcard .row.purchase_entries{border:none;}';var scroll_color='.dycolor{background:'+res.userbadge.bg_color+'!important;}.image_background{background:'+res.userbadge.bg_color+'}.Clubcard .card .item:last-child, .Clubcard .list-inset .item:last-child{'+res.userbadge.bg_color+'}';var css=document.createElement("style");css.type="text/css";css.innerHTML=scroll_color;document.body.appendChild(css);$scope.canvasBar(res.userpoints||0,res.maxpoints,res.cards,res.progressbar_color);Loader.hide()})}
$scope.showTC=function(){Modal.fromTemplateUrl('templates/modal.html',{scope:$scope,animation:'slide-in-up'}).then(function(modal){$scope.modal=modal;$scope.modal.show()})}
if(!Customer.isLoggedIn()){Customer.loginModal($scope,function(res){$scope.user.id=Customer.customer.id;$scope.user.fname=Customer.customer.firstname;$scope.user.lname=Customer.customer.lastname;$scope.loadContent()},function(){$state.go("clubcard-front",{value_id:$scope.value_id})});Loader.hide()}else{$scope.user.id=Customer.customer.id;$scope.user.fname=Customer.customer.firstname;$scope.user.lname=Customer.customer.lastname;$scope.loadContent()}
window.onresize=function(){document.querySelector('.auto_height').style.minHeight=window.innerHeight+'px'}});angular.module('starter').factory('Clubcard',function($sbhttp,$http,$rootScope,Url){var factory={};factory.value_id=null;factory.getpoints=function(id){var params={user_id:id,value_id:this.value_id};return $sbhttp({method:'GET',url:Url.get('clubcard/mobile_view/find',params),cache:!1,responseType:'json'})};return factory});Features.insertCSS(".Clubcard .card{max-height:200px;box-shadow:none}.Clubcard .card,.Clubcard .list-inset{margin:unset;overflow:visible}.Clubcard h1,.Clubcard h2,.Clubcard h3,.Clubcard h4,.Clubcard h5,.Clubcard h6{text-align:center}.Clubcard .item-image .list-img,.Clubcard .item-image img:first-child{max-height:200px}.Clubcard .loader{background:#287A51!important;border-radius:100px;height:20px}.Clubcard .loader p{text-align:center;color:white!important}.Clubcard .card .item:last-child,.Clubcard .list-inset .item:last-child{box-shadow:none}.Clubcard .progressbadge{position:absolute}.Clubcard img.cardimage{margin-top:-10px!important;margin-right:-25px!important;position:absolute}.Clubcard .bgwhite{background:#fff;border-radius:15px}.Clubcard .tooltip{position:relative}.Clubcard .tooltip .tooltiptext{color:#111;text-align:right;z-index:1;left:32%;position:absolute;top:-71px}.Clubcard .tooltip .tooltiptext::after{content:\"|\";position:absolute;top:50%;left:14%;margin-top:-5px;font-size:65px;color:green;font-weight:100}.Clubcard .prog_margin{margin-top:35px}.Clubcard .nocover{height:200px;max-height:200px;width:100%}.Clubcard .tooltip .tooltiptext{display:inline-block;width:100px;text-align:-webkit-left}.Clubcard .tooltip .tooltiptext::after{margin-top:0;font-size:85px;z-index:0}.Clubcard img.cardimage{z-index:10}.Clubcard span.tooltiptext.left_after::after{left:-7px}.Clubcard .row.badgename h3{text-align:center;width:100%;margin:10px 0}.Clubcard .row.badgename_image{display:inline-block;text-align:center;margin-bottom:25px}.Clubcard .badgename_image img{display:inline-block;vertical-align:middle}.Clubcard .badgename_image{display:inline-block;vertical-align:middle}.Clubcard .prog_margin .item.item-text-wrap{padding-bottom:5px}.Clubcard .purchase_entries h4{text-align:center;display:inline-block;vertical-align:middle;margin:0}.Clubcard .purchase_entries h1{margin:0;display:inline-block;vertical-align:top}.Clubcard .row.purchase_entries{border-bottom:1px solid;margin:0 0 14px;padding:6px 0}.Clubcard p.next_visit{text-align:center}.Clubcard .prog_margin .item.item-text-wrap{padding-bottom:5px;max-width:320px;width:100%;margin:0 auto}.Clubcard .left_text>b{margin-left:-10px}.Clubcard .left_text>span{margin-left:-62px}.Clubcard .scroll{background:#fff!important}canvas#progressCanvas{width:125%!important}.row.purchase_entries.no_border{border:none}h6.before_point.ng-binding{margin-top:-50px}.row.note-privilege{min-height:150px}.always_bottom{position:relative!important;bottom:-5%}html,body{height:100%;width:100%}.auto-height{position:relative}h5.before_point{margin-top:-50px}","clubcard")